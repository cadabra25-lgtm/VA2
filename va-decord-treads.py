...
# ==============================
def process_single_video(video_path, start_minute, skip_frames, batch_size, roi, common_log_path):
    video_name = os.path.basename(video_path)
    try:
        ROI = roi
        vr = VideoReader(video_path, ctx=cpu(0))
        #vr = VideoReader(video_path, ctx=cuda(0))
...
# ==============================
def process_folder(folder_path, start_minute, skip_frames, batch_size, roi):
    ...
    for idx, video_path in enumerate(mp4_files, start=1):
        ...
        result = process_single_video(video_path, start_minute, skip_frames, batch_size, roi, common_log_path)
...
class App:
    def __init__(self, root):
        ...
        self.folder_path = tk.StringVar(value=r"C:\video-test\cams\cam1-test")
    ...
    def start_processing(self):
        try:
            ...
            if skip < 1 or batch < 1:
                messagebox.showwarning("Ошибка", "Пропуск кадров и размер батча должны быть >= 1")
                return
            thread = threading.Thread(
                target=process_folder,
                args=(folder, start_min, skip, batch, self.roi),
                daemon=True
            )
            thread.start()
        except ValueError:
            messagebox.showerror("Ошибка", "Проверьте, что все параметры — целые числа.")
